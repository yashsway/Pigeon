...
function reportEntry(){
    ...
    if($GLOBALS['displayReports']==true){
        for($i=0;$i<count($reports);$i++){
            ...
            echo '<td class="report-elements report-title text">' . htmlspecialchars($reports[$i]->summary,ENT_QUOTES,'UTF-8') . '</td>';
            echo '<td class="report-elements report-date text">' . htmlspecialchars($reports[$i]->_date,ENT_QUOTES,'UTF-8') . '</td>';
            echo '<td class="report-elements report-duration text">' . htmlspecialchars($reports[$i]->duration . ' day(s)',ENT_QUOTES,'UTF-8') . '</td>';
