...
function tableFormatter(val, container, type) {
    switch (type) {
    case 0:
        if (container == "reportID") {
            ...
            return '<td class="report-elements report-icons report-tag">' + tagCodeGenerator(val) + '</td>';
        } else if (container == "reportDate") {
            return '<td class="report-elements report-date text">' + val + '</td>';
        } else if (container == "reportPriority") {
...
$("#newReport_clear").click(function () {
    $("#newReport_name").val('');
    $("#newReport_name").parent().css('background-color', '');
    $("#newReport_phone").val('');
    $("#newReport_phone").parent().css('background-color', '');
    $("#newReport_email").val('');
    $("#newReport_email").parent().css('background-color', '');
    $("#newReport_department").val('blank');
    $("#newReport_department").parent().css('background-color', '');
    $("#newReport_requestCategory").val('blank');
    $("#newReport_requestCategory").parent().css('background-color', '');
    $("#newReport_otherRequest").val('');
    $("#newReport_otherRequest").parent().css('background-color', '');
    ...
    $('#newReport_date').val('');
    $('#newReport_date').css('background-color', '');
    $('#newReport_time').val('');
    $('#newReport_time').css('background-color', '');
    $("#newReport_otherRequest").prop('disabled', true);
...
function newReport_compilation() {
    ...
    var formData = {
        reqType: 2,
        id: hash(),
        na: $("#newReport_name").val(),
        ph: $("#newReport_phone").val() + "  x" + $("#newReport_extension").val(),
        em: $("#newReport_email").val(),
        dep: $("#newReport_department").val(),
        req: $("#newReport_requestCategory").val(),
        cus: $("#newReport_otherRequest").val(),
        summ: $("#newReport_summary").val(),
        pri: priorityNumberGenerator($("input[type='radio'][name='priority']:checked").val()),
        dat: $("#newReport_date").val(),
        tim: $("#newReport_time").val()
    };
    //TEST: progress bar
...
function editReport_compilation(id) {
    ...
    if ($("input[type='radio'][name='adminPriority']:checked").val() != undefined) {
        var formData = {
            reqType: 4,
            id: id,
            summ: $("#editReport_summary").val(),
            na: $("#editReport_name").val(),
            ph: $("#editReport_phone").val(),
            em: $("#editReport_email").val(),
